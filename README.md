...
1. gene_expression_file: Gene expression matrix derived from omics data (e.g., RNA-seq or proteomics).
   - Rows: genes (gene symbols)
   - Columns: samples (sample IDs in the first row)

2. metabolite_expression_file: Metabolite expression matrix from metabolomics data.
   - Rows: metabolites (unified metabolite names)
   - Columns: samples (sample IDs in the first row)

3. meta_gene_relation_file: A curated mapping file describing, for each target metabolite:
...
4. ESTIMATE_score_results: A matrix containing four inferred tumor microenvironment scores generated by the ESTIMATE algorithm:
   - Stromal score
   - Immune score
   - ESTIMATE score
   - Tumor purity
